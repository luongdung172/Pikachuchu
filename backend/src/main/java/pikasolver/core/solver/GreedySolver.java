...
import pikasolver.core.model.SolverStats;

import java.util.*;

public class GreedySolver implements Solver{
    private final MoveFinder moveFinder;
    public GreedySolver(MoveFinder moveFinder) {
        if (moveFinder == null) {
    ...
    public SolveResult solve(Board originalBoard) {
        ...
        while (!isSolved(board)) {
            ...
            Move bestMove = chooseBestMove(validMoves);

            board.removePair(bestMove.getFrom(), bestMove.getTo());
    ...
    private Move chooseBestMove(List<Move> validMoves) {
        return validMoves.get(0);
    }

    private boolean isSolved(Board board) {
        return countRemainingPairs(board) == 0;
    }
